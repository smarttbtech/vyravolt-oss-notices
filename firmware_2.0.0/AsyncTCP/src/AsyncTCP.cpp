...
static inline bool _send_async_event(lwip_event_packet_t** e, TickType_t wait = pdMS_TO_TICKS(10)) {
...
// In LwIP Thread (gehärtete Version)
int8_t AsyncClient::_lwip_fin(tcp_pcb* pcb, int8_t err) {

  tcp_pcb* local = _pcb;

  // Wenn kein PCB mehr oder falscher PCB → sauber raus
  if (!local || pcb != local) {
    log_d("FIN ignore: 0x%08x != 0x%08x", (uint32_t)pcb, (uint32_t)local);
    return ERR_OK;
  }

  // SOFORT entkoppeln (Race-Fenster schließen)
  _pcb = NULL;

  // Callbacks im lwIP Thread entfernen
  tcp_arg(local, NULL);
  tcp_sent(local, NULL);
  tcp_recv(local, NULL);
  tcp_err(local, NULL);
  tcp_poll(local, NULL, 0);

  // PCB sauber schließen
  if (tcp_close(local) != ERR_OK) {
    tcp_abort(local);
  }
...
int8_t AsyncServer::_s_accept(void* arg, tcp_pcb* pcb, int8_t err) {
  AsyncServer* server = reinterpret_cast<AsyncServer*>(arg);
  if (!server) {
    if (pcb) {
      if (tcp_close(pcb) != ERR_OK) {
        tcp_abort(pcb);
    ...
    log_d("s_accept called with NULL server arg");
    return ERR_OK;
  }
...
int8_t AsyncServer::_s_accepted(void* arg, AsyncClient* client) {
  ...
  if (!server) {
    ...
    log_d("s_accepted called with NULL server arg");
    return ERR_OK;
  }
  return server->_accepted(client);
}
