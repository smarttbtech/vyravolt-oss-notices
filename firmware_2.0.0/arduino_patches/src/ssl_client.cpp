...
int data_to_read(sslclient_context *ssl_client) {
  ...
  // Some backends close keep-alive TLS sockets abruptly after delivering
  // the complete HTTP response. This can surface as -76 in a non-blocking
  // available() probe and is harmless for one-shot HTTP requests.
  if (ret == -76) {
    return 0;
  }
  ...
  return res + (ssl_client->peek_buf >= 0);
}
